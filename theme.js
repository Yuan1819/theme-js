...
theme.collectionView = (function() {
  function collectionView(container) {
    ...
    ajaxfilter = (function(url, filterTags){
        ...
        $.ajax({
          ...
          complete: function (data) {
            ...
            console.log('filteredTags ', filterTags);
            let filteredProductsArray = $('.productList .grid--view-items .grid__item');
            console.log('filteredProductsArray ', filteredProductsArray);
            filteredProductsArray.each((i, obj) => {
              var item_variants = $(obj).data("variants");
              var current_product = $(obj);
              var isOutofStock = false;
              filterTags.forEach((tag) => {
                if(isOutofStock) return;
                item_variants.forEach((item) => {
                  if(isOutofStock) return;
                  // console.log('item.option1', item.option1)
                  if (item.option1.toLowerCase().includes(tag.toLowerCase()) && !item.available) {
                    // current_product.addClass('hide');
                    current_product.remove();
                    isOutofStock = true;
                    console.log('removed product', current_product)
                    console.log("removed product's item_variants ", item_variants);
                  }
                });
              })
            });
...
theme.ajaxCart = function(){
        ...
        $(document).on('click touchend', '.product-form__cart-submit', function(i){
          if (i.preventDefault(), "disabled" != $(this).attr("disabled"))
              if (touchmoved != true) {
                ...
                var tags_json = tags.split(',');
                // const addon = tags_json.map(element => element.includes('addon-'))
                const addon = tags_json.filter(function(element) {
                  return element.includes('addon-');
                });
                var addon_handle = [];
                if(addon){
                  addon.forEach(function(element) { 
                    var single_addon = element.replace('addon-', '');
                    addon_handle.push(single_addon);
                  });
                  
                }
                console.log('addon_handle ', addon_handle);
                addinToCart(att, qty, props, addon_handle);
              }
              return;
    ...
    function addinToCart(att, qty, props, addon_handle){
      $("body").addClass("loading");
      CartJS.addItem(att, qty,props,{
          "success": function(data, textStatus, jqXHR){
             ...
             if(addon_handle.length > 0){
              var promises = [];
              addon_handle.forEach(function(handle){
                var addon_quick_view_url = '/products/' + handle + '?view=addon_quick_view';
                promises.push(
                  $.ajax({
                    url: addon_quick_view_url,
                    success: function(data) {
              ...
              Promise.all(promises).then((product) =>{
                  if(product.length > 0){
                    var bundle_products = product.join(' ');
                    $.magnificPopup.open({
                      items: {
                        src: `<div class="product-page-bundle quick-view-popup mfpbox mfp-with-anim item-length-${product.length}" id="content_quickview_bundle"><h2 class="bundle-title">Complete The Look</h2><p class="bundle-desc">Over 15,000 Customers Chose to Complete The Look</p><div class="bundle__wrapper">${bundle_products}</div></div>`,
                        type: 'inline'
                      },
                      removalDelay:500,
                      callbacks: {
                        ...
                        close: function() {
                          $('.stickyHeader').removeClass('popup');
                          $( '#content_quickview_bundle' ).empty();
                        }
                      },
                    })
                  }
